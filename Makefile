...
GIT_VERSION          		?= $(shell echo `git describe --match=NeVeRmAtCh --always`)
...
GIT_BRANCH            := $(shell echo $(GIT_BRANCH) | sed "s,/,-,g" | sed "s,_,-,g")
...
scan: 
	@echo "performing vulnerability scan for container image $(SERVICE):${FULL_VERSION}"
	trivy image $(DOCKER_INT_REGISTRY)/$(SERVICE):$(FULL_VERSION)
...
ifeq ($(CHART_INSTALLED),)
	...
	KUBECONFIG=${KUBECONFIG} helm install charts/build/$(SERVICE) --wait --name $(SERVICE) --version $(FULL_VERSION) --set image.tag=$(FULL_VERSION)
else
	KUBECONFIG=${KUBECONFIG} helm upgrade $(SERVICE) charts/build/$(SERVICE) --wait --install --version $(FULL_VERSION) --set image.tag=$(FULL_VERSION)
endif
